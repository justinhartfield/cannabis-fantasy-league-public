 ...
 * 
 * View for a single manufacturer with score chart and trading
 */
...
const PLACEHOLDER_IMG = "https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?w=400&h=400&fit=crop";

export default function ManufacturerDetail() {
    const params = useParams<{ id: string }>();
    const [, setLocation] = useLocation();
    const { user } = useAuth();
    const manufacturerId = parseInt(params.id || "0");
    ...
    const { data: mfg, isLoading, error } = trpc.stockMarket.getManufacturerDetail.useQuery(
        { manufacturerId },
    ...
    if (error || !mfg) {
        return (
            <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center gap-4">
                <p className="text-zinc-400">Manufacturer not found</p>
    ...
    const isPositive = mfg.scoreChange >= 0;

    // Calculate chart dimensions
    const chartData = mfg.scoreHistory;
    const scores = chartData.map(d => d.score);
    const maxScore = Math.max(...scores, mfg.currentScore);
    const minScore = Math.min(...scores, mfg.currentScore);
        ...
        <div className="min-h-screen bg-zinc-950 text-white">
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-12 gap-8 mb-8">
                    ...
                    <div className="col-span-12 lg:col-span-4">
                        <div className="w-full aspect-square rounded-2xl overflow-hidden bg-gradient-to-br from-zinc-800 to-zinc-900 ring-2 ring-zinc-700">
                            <img
                                src={mfg.logoUrl || PLACEHOLDER_IMG}
                                alt={mfg.name}
                        ...
                        <div className="h-full flex flex-col">
                            ...
                            <div className="mb-6">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-sm text-zinc-400 uppercase tracking-wide">Manufacturer</span>
                                </div>
                                <h1 className="text-4xl font-bold text-white mb-2">{mfg.name}</h1>
                                <div className="flex items-center gap-4">
                                    <span className="text-5xl font-black text-white">
                                        {Math.round(mfg.currentScore)}
                                    </span>
                                    ...
                                    <div className={cn(
                                        ...
                                        {isPositive ? '+' : ''}{mfg.scoreChange} ({mfg.scoreChangePercent}%)
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                <div className="rounded-xl p-4 ring-1 bg-emerald-500/10 text-emerald-400 ring-emerald-500/20">
                                    ...
                                    <div className="text-2xl font-bold text-white">{mfg.todayOrders.toLocaleString()}</div>
                                </div>
                                <div className="rounded-xl p-4 ring-1 bg-yellow-500/10 text-yellow-400 ring-yellow-500/20">
                                    ...
                                    <div className="text-2xl font-bold text-white">#{mfg.currentRank}</div>
                                    {mfg.previousRank > 0 && (
                                        <div className="text-xs mt-1 opacity-70">
                                            {mfg.currentRank < mfg.previousRank ? `↑ from #${mfg.previousRank}` :
                                                mfg.currentRank > mfg.previousRank ? `↓ from #${mfg.previousRank}` : 'No change'}
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        {chartData.length > 0 ? (
                            <div className="h-64 relative">
                                <svg viewBox="0 0 800 200" className="w-full h-full" preserveAspectRatio="none">
                                    <defs>
                                        <linearGradient id="mfgGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                ...
                {mfg.description && (
                    <Card className="bg-zinc-900/50 border-zinc-800 mb-8">
                        <CardContent className="p-6">
                            <h3 className="text-lg font-semibold text-white mb-3">About {mfg.name}</h3>
                            <p className="text-zinc-400 leading-relaxed">{mfg.description}</p>
                        </CardContent>
                    </Card>
                )}
            </div>
                ...
                <DialogContent className="bg-zinc-900 border-zinc-800 text-white">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            {tradeModal.action === 'buy' ? 'Buy' : 'Sell'} {mfg.name}
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-4">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-xl font-bold">{Math.round(mfg.currentScore)} pts</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-zinc-800">
                            ...
                            <span className="text-2xl font-bold text-white">
                                {Math.round(mfg.currentScore * tradeModal.shares)} pts
                            </span>
                        </div>
                    </div>

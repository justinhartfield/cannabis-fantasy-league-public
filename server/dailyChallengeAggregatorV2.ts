...
export class DailyChallengeAggregatorV2 {
  ...
  private async fetchBrandRatings(statDate: string): Promise<Array<{
    ...
    try {
      ...
      // If no brand reviews found, log what review types ARE present
      if (brandRowCount === 0 && rawRows.length > 0) {
        const reviewTypes = new Set(rawRows.map(r => this.getFieldCaseInsensitive(r, 'ReviewType')).filter(Boolean));
        console.warn(`[Brand Ratings] No brand reviews found. Available review types: ${Array.from(reviewTypes).join(', ')}`);
      }

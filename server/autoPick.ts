...
import { eq, and, notInArray, inArray, desc, isNotNull, sql } from "drizzle-orm";
...
class AutoPickService {
  ...
  private async getBestManufacturer(
    ...
    let results = await db
      ...
      .where(
        and(
          excludedIds.length > 0 ? notInArray(manufacturers.id, excludedIds) : undefined,
          sql`${manufacturers.productCount} > 0`
        )
      )
...
<<<<<<< Updated upstream
=======
  if (process.env.USE_LEGACY_AUTOPICK === "1") {
    return makeAutoPickLegacy(leagueId, teamId);
  }
...
/**
 * Original makeAutoPick implementation (preserved for emergency rollback)
 */
async function makeAutoPickLegacy(leagueId: number, teamId: number): Promise<void> {
  console.log(`[AutoPick Legacy] makeAutoPick called for league ${leagueId}, team ${teamId}`);
>>>>>>> Stashed changes
  const db = await getDb();
...
=======
  const positions = ["manufacturer", "cannabis_strain", "product", "pharmacy", "brand"] as const;
  const sortedPositions = [...positions].sort((a, b) => {
    return positionCounts[a] - positionCounts[b];
  });
>>>>>>> Stashed changes
...
=======
  for (const position of sortedPositions) {
    ...
    if (position === "manufacturer") {
      ...
      // Note: strains table doesn't have imageUrl column
      const productResults = await db
        ...
        .orderBy(desc(productDailyChallengeStats.totalPoints), desc(strains.pharmacyCount), desc(strains.favoriteCount))
        .limit(1);
      available = productResults.map(r => ({ ...r, imageUrl: null }));
    ...
    if (available.length > 0) {
      pickedAsset = {
        ...
        imageUrl: available[0].logoUrl || available[0].imageUrl || null,
        type: position
      };
...
=======
    const currentPick = draftStatus[0].currentDraftPick;
    const existingPick = await db
      .select()
      .from(draftPicks)
      .where(
